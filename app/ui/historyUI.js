...
export function renderSidebarHistoryList() {
  ...
  if (!currentNote.savedPoints || currentNote.savedPoints.length === 0) {
    container.innerHTML =
      '<div class="history-empty" style="font-size: 12px; padding: 20px; text-align: center; color: var(--text-secondary);">履歴なし</div>';
    return;
  }
  ...
  recentPoints.forEach((sp) => {
    ...
    div.innerHTML = `
      ...
      <div class="history-actions" style="margin-top: 5px; display: flex; gap: 4px;">
        <button class="restore-history-btn" style="flex: 1; padding: 4px; font-size: 10px;"><span class="material-symbols-outlined">restore</span> 復元</button>
        <button class="view-detail-btn" style="flex: 1; padding: 4px; font-size: 10px;"><span class="material-symbols-outlined">visibility</span> 詳細</button>
...
export function openHistoryModal() {
  ...
  const renderHistoryList = () => {
    ...
    if (!currentNote.savedPoints || currentNote.savedPoints.length === 0) {
      historyListEl.innerHTML =
        '<li class="history-empty">保存された履歴がありません</li>';
      return;
    }
    ...
    currentNote.savedPoints.forEach((sp) => {
      ...
      li.innerHTML = `
        ...
        <div class="history-actions">
          <button class="restore-history-btn"><span class="material-symbols-outlined">restore</span> 復元</button>
          <button class="view-detail-btn"><span class="material-symbols-outlined">visibility</span> 詳細</button>
...
export function showHistoryDetail(savedPoint) {
  ...
  data.forEach((item) => {
    ...
    const amount = item.kind * count;
    const formattedAmount =
      currentNote.currency === "JPY"
        ? `${amount.toLocaleString()}円`
        : `${amount.toLocaleString(undefined, { maximumFractionDigits: 2 })}元`;
    html += `<tr><td class="detail-label">${item.label}</td><td class="detail-count">${count}枚</td><td class="detail-amount">${formattedAmount}</td></tr>`;
  });
  html += `</tbody></table>`;
