...
切り替え一覧は、通常時にノート名とピンのみを表示します。「整理」ではチェック選択し、1件なら上下移動・削除、複数件なら削除だけを表示します。編集は各ノートの鉛筆ボタンから開きます。ピン付きの行には「固定」と色を付けて表示します。ノートの配列順を手動順として保存し、`pinned` が真のノートを先頭に表示します。旧バックアップで `pinned` がないノートは通常ノートとして扱います。PC サイドバーも同じ表示順です。
