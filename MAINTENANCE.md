...
切り替え一覧は、通常時にノート名とピンのみを表示します。「整理」ではチェック選択し、1件の上下移動・編集、選択した複数件の削除ができます。ノートの配列順を手動順として保存し、`pinned` が真のノートを先頭に表示します。旧バックアップで `pinned` がないノートは通常ノートとして扱います。PC サイドバーも同じ表示順です。
